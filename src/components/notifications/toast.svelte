...
afterUpdate(() => {
  ...
  export let isShow; 
  export let toggleToast;  
  
</script> 
<div class={`absolute top-0 right-0 m-3 w-2/3 md:w-1/3 ${!isShow ? "hidden" : " "}`} x-transition:enter="transition transform ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition transform ease-in duration-300" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-1">
  <div class="bg-white border-gray-300 border p-3 flex items-start shadow-lg rounded-md space-x-2">
    ...
    <div class="flex-1 space-y-1">
      ...
      <p class="text-sm leading-5 text-gray-600">Saved as...</p>
    </div>
    <svg class="flex-shrink-0 h-5 w-5 text-gray-400 cursor-pointer" on:click={toggleToast} stroke="currentColor" viewBox="0 0 20 20">
